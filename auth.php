 ...
 */
class auth_plugin_onedevice extends auth_plugin_base
{
    ...
    {
        $this->authtype = 'onedevice';
        $this->config = get_config('auth_onedevice');
    }
     ...
     */
    public function auth_plugin_onedevice()
    {
    ...
    function user_login($username, $password)
    {
        ...
        if ($user = $DB->get_record('user', array('username' => $username, 'mnethostid' => $CFG->mnet_localhost_id))) {
            if (validate_internal_user_password($user, $password)) {
                if ($_REQUEST["device"]["uuid"]) {
                    if ($user = $DB->get_record('auth_onedevice', array('username' => $username))) {
                        return $_REQUEST["device"]["uuid"] === $user->deviceid;
                    } else {
                        $DB->insert_record('auth_onedevice', (object)array('username' => $username, 'deviceid' => $_REQUEST["device"]["uuid"]));
                        return true;
                    }
//                } else {
//                    return !isset($_REQUEST["service"]);
    ...
    public function user_signup_with_confirmation($user, $notify=true, $confirmationurl = null) {
        ...
        if (!empty($SESSION->wantsurl)) {
            set_user_preference('auth_onedevice_wantsurl', $SESSION->wantsurl, $user);
        }
        ...
        if (! send_confirmation_email($user, $confirmationurl)) {
            print_error('auth_onedevicenoemail', 'auth_onedevice');
        }
    ...
    function user_confirm($username, $confirmsecret) {
        ...
        if (!empty($user)) {
            if ($user->auth != $this->authtype) {
                ...
                if ($wantsurl = get_user_preferences('auth_onedevice_wantsurl', false, $user)) {
                    // Ensure user gets returned to page they were trying to access before signing up.
                    $SESSION->wantsurl = $wantsurl;
                    unset_user_preference('auth_onedevice_wantsurl', $user);
                }
    ...
    {
        global $DB;
        $DB->delete_records('auth_onedevice', array('username' => $user->username));
    }
